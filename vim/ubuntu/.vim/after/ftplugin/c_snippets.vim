...
exec "Snippet do do{<CR><TAB>".st.et."<CR>} while (".st.et.");".st.et
exec "Snippet readfile std::vector<uint8_t> v;<CR>if(FILE* fp = fopen(\"".st."filename".et."\", \"r\")){<CR><TAB>uint8_t buf[1024];<CR>while(size_t len = fread(buf, 1, sizeof(buf), fp))<CR>v.insert(v.end(), buf, buf + len);<CR>fclose(fp);<CR><BS>}<CR>".st.et
...
exec "Snippet class class ".st."name".et."{<CR><TAB>public:<CR>".st."name".et." (".st."arguments".et.");<CR>virtual ~".st."name".et."();<CR><CR>private:<CR>".st.et."<CR>};<CR>".st.et
...
exec "Snippet struct struct ".st."name".et."{<CR><TAB>".st.et."<CR>};<CR>".st.et
...
exec "Snippet namespace namespace ".st.":substitute(expand('%'),'.','\\l&', 'g')".et."{<CR><TAB>".st.et."<CR>};<CR>".st.et
...
exec "Snippet if if(".st.et."){<CR><TAB>".st.et."<CR><BS>}<CR>".st.et
exec "Snippet main int main (int argc, char const* argv[]){<CR><TAB>".st.et."<CR>return 0;<CR><BS>}<CR>".st.et
exec "Snippet Inc #include <".st.et."><CR>".st.et
exec "Snippet inc #include \"".st.et.".h\"".st.et
exec "Snippet for for( ".st.et." ".st."i".et." = ".st.et."; ".st."i".et." < ".st."count".et."; ".st."i".et." += ".st.et."){<CR><TAB>".st.et."<CR><BS>}<CR>".st.et
